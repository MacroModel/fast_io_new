...
inline constexpr ::fast_io::freestanding::array<char_type, N> characters_array_impl{
...
inline constexpr ::fast_io::freestanding::array<char_type, N> wide_characters_array_impl{
	create_find_simd_vector_with_unsigned_toggle<signed_disposition, char_type, N>(lfch)};
...
inline constexpr char_type const *find_simd_common_condition_impl(char_type const *first, char_type const *last,
																  Func func) noexcept
{
...
template <bool findnot, ::std::size_t vec_size, ::std::integral char_type, typename Func>
	requires(sizeof(char_type) <= vec_size) && (vec_size % sizeof(char_type) == 0)
inline constexpr char_type const *find_simd_common_all_impl(char_type const *first, char_type const *last,
															Func func) noexcept
{
	constexpr unsigned N{vec_size / sizeof(char_type)};
	using simd_vector_type = ::fast_io::intrinsics::simd_vector<char_type, N>;
	if constexpr (findnot)
	{
		...
		return find_simd_common_condition_impl<vec_size>(first, last,
														 [&](simd_vector_type const &simdvec) noexcept -> bool { return !is_all_zeros(func(simdvec) == zero_vecs); });
	}
	else
	{
		return find_simd_common_condition_impl<vec_size>(first, last,
														 [&](simd_vector_type const &simdvec) noexcept -> bool { return !is_all_zeros(func(simdvec)); });
	}
}

template <bool findnot, ::std::size_t vec_size, ::std::integral char_type, typename simd_vector_type>
inline constexpr char_type const *find_simd_constant_simd_common_all_impl(char_type const *first, char_type const *last,
																		  simd_vector_type const &charsvec) noexcept
{
	return find_simd_common_condition_impl<vec_size>(first, last,
													 [&](simd_vector_type const &simdvec) noexcept -> bool {
														 if constexpr (findnot)
														 {
...
inline constexpr char_type const *find_simd_constant_simd_common_impl(char_type const *first,
...
inline constexpr wchar_t const *find_space_simd_common_wide_none_utf_encoding_impl(wchar_t const *first,
																				   wchar_t const *last) noexcept
{
	...
	using decision_simd_vector_type =
		::std::conditional_t<use_signed_vector_type, signed_simd_vector_type, unsigned_simd_vector_type>;

	constexpr char_type five{5};
#if (__cpp_lib_bit_cast >= 201806L) && !defined(__clang__)
	constexpr decision_simd_vector_type fives{::std::bit_cast<decision_simd_vector_type>(
		wide_characters_array_impl<five, char_type, N, use_signed_vector_type>)};
	constexpr decision_simd_vector_type horizontaltabs{::std::bit_cast<decision_simd_vector_type>(
		wide_characters_array_impl<horizontaltab, char_type, N, use_signed_vector_type>)};
#else
...
#endif
#if (__cpp_lib_bit_cast >= 201806L) && !defined(__clang__)
	constexpr simd_vector_type spaces{
	...
	{
#if (__cpp_lib_bit_cast >= 201806L) && !defined(__clang__)
		constexpr simd_vector_type verticaltabs{
...
#endif
		return find_simd_common_all_impl<findnot, vec_size>(
			first, last,
			[&](simd_vector_type const &simdvec) noexcept {
				if constexpr (::std::same_as<wchar_t, char_type> && wide_is_none_utf_endian)
				{
					...
					return (spaces == simdvec) ^
						   (static_cast<simd_vector_type>(
								(static_cast<decision_simd_vector_type>(simdvec_reverse) - horizontaltabs) < fives) &
							(simdvec != verticaltabs));
				}
				else
				{
					return (spaces == simdvec) ^
						   (static_cast<simd_vector_type>(
								(static_cast<decision_simd_vector_type>(simdvec) - horizontaltabs) < fives) &
							(simdvec != verticaltabs));
				}
			});
	}
	...
	else
	{
		...
		if constexpr (is_ebcdic<char_type>)
		{
			...
			*/
...
#if (__cpp_lib_bit_cast >= 201806L) && !defined(__clang__)
				constexpr simd_vector_type formfeeds{
					::std::bit_cast<simd_vector_type>(characters_array_impl<formfeed, char_type, N>)};
				constexpr simd_vector_type carriagereturns{
			...
			{
#if (__cpp_lib_bit_cast >= 201806L) && !defined(__clang__)
				constexpr decision_simd_vector_type threes{::std::bit_cast<decision_simd_vector_type>(
					characters_array_impl<three, char_type, N, use_signed_vector_type>)};
				constexpr decision_simd_vector_type verticaltabs{::std::bit_cast<decision_simd_vector_type>(
					characters_array_impl<verticaltab, char_type, N, use_signed_vector_type>)};
#else
				...
				return find_simd_common_all_impl<findnot, vec_size>(
					...
					[&](simd_vector_type const &simdvec) noexcept {
						return (horizontaltabs == simdvec) ^
							   static_cast<simd_vector_type>(
								   (static_cast<decision_simd_vector_type>(simdvec) - verticaltabs) < threes) ^
							   (ebcdic_specific_nls == simdvec) ^ (linefeeds == simdvec) ^ (spaces == simdvec);
					});
			}
		}
...
#if (__cpp_lib_bit_cast >= 201806L) && !defined(__clang__)
			constexpr decision_simd_vector_type fives{::std::bit_cast<decision_simd_vector_type>(
				characters_array_impl<five, char_type, N, use_signed_vector_type>)};
			constexpr decision_simd_vector_type horizontaltabs{::std::bit_cast<decision_simd_vector_type>(
				characters_array_impl<horizontaltab, char_type, N, use_signed_vector_type>)};
#else
			decision_simd_vector_type fives;
														   ...
														   char unsigned ch) noexcept
{
...
#endif
	{
		...
		{
			for (::std::size_t const constantk{ones * ch}; diff <= static_cast<::std::size_t>(last - first);
				 first += diff)
			{
...
inline constexpr char_type const *find_simd_constant_common_cold_impl(char_type const *first,
																	  char_type const *last) noexcept
{
	constexpr char_type lfchct{char_literal_v<lfch, ::std::remove_cvref_t<char_type>>};
...
#endif
	{
		...
		if constexpr (use_builtin_memchr &&
					  (sizeof(char_type) == 1 || (sizeof(char_type) == 4 && sizeof(wchar_t) == 4)))
		{
		...
		{
			first = find_simd_constant_simd_common_impl<
				lfch, findnot, ::fast_io::details::optimal_simd_vector_run_with_cpu_instruction_size>(first, last);
		}
		else if constexpr (sizeof(char_type) == 1)
		{
			char unsigned const *firstconstptr{reinterpret_cast<char unsigned const *>(first)};
			return find_characters_musl<findnot>(firstconstptr, reinterpret_cast<char unsigned const *>(last), lfchct) -
				   firstconstptr + first;
		}
	}
#endif
...
find_simd_small_optimization_common_impl(char_type const *first, char_type const *last, Pred pred, Func func) noexcept
{
...
{
	return find_simd_small_optimization_common_impl<false>(
		first, last,
		...
		[](char_type const *first2, char_type const *last2) noexcept { return find_simd_constant_common_cold_impl<lfch, findnot>(first2, last2); });
}
...
inline constexpr char_type const *find_space_common_impl(char_type const *first, char_type const *last) noexcept
{
	return find_simd_small_optimization_common_impl<false>(
		...
		[](char_type const *first2, char_type const *last2) noexcept { return find_space_common_cold_impl<ishtml, findnot>(first2, last2); });
}
...
inline constexpr Iter find_space_impl(Iter first, Iter last)
{
	...
	if constexpr (!::std::is_volatile_v<value_type> && ::std::contiguous_iterator<Iter>)
	{
		...
		else if constexpr (::std::is_pointer_v<Iter>)
		{
			...
			return ::fast_io::details::find_space_common_impl<ishtml, findnot>(first_const_ptr, last) -
				   first_const_ptr + first;
		}
		else
		{
			::std::remove_cvref_t<value_type> const *first_const_ptr{::std::to_address(first)};
			return ::fast_io::details::find_space_common_impl<ishtml, findnot>(first_const_ptr,
																			   ::std::to_address(last)) -
				   first_const_ptr + first;
		}
	}
...
inline constexpr Iter find_ch_impl(Iter first, Iter last)
{
	...
	if constexpr (!::std::is_volatile_v<value_type> && ::std::contiguous_iterator<Iter>)
	{
		...
		{
			::std::remove_cvref_t<value_type> const *first_const_ptr{first};
			return ::fast_io::details::find_simd_constant_common_impl<lfch, findnot>(first_const_ptr, last) -
				   first_const_ptr + first;
		}
		else
		{
			::std::remove_cvref_t<value_type> const *first_const_ptr{::std::to_address(first)};
			return ::fast_io::details::find_simd_constant_common_impl<lfch, findnot>(first_const_ptr,
																					 ::std::to_address(last)) -
				   first_const_ptr + first;
		}
	}
