	...
	bool &source_eof, ::std::size_t preferred)
{
					  ...
					  has_ibuffer_basic_operations<input_ref>)
	{
		...
		if (current == end)
		{
			...
			current = ibuffer_curr(inref);
			end = ibuffer_end(inref);
			if (current == end)
			{
	...
	else
	{
						  ...
						  unit_type, typename input_ref::input_char_type>)
		{
			...
			next = ::fast_io::operations::decay::read_some_decay(
							   ...
							   has_ibuffer_basic_operations<input_ref>)
		{
			...
			::fast_io::freestanding::my_memcpy(
				scratch.buffer_begin, current, bytes);
			ibuffer_set_curr(inref, current + units);
		...
		else
		{
			...
			next = ::fast_io::operations::decay::read_some_bytes_decay(
				inref, scratch.buffer_begin, scratch.buffer_capacity_end);
		}
		if (next == scratch.buffer_begin)
		{
		...
		if (next < scratch.buffer_begin ||
			next > scratch.buffer_capacity_end)
		{
			// Reject a source primitive returning a cursor outside its buffer.
			::fast_io::throw_transcode_stream_error(
				::fast_io::transcode_stream_errc::protocol_violation);
		}
...
inline void commit_transcode_source(
	input_ref &inref,
	::fast_io::details::basic_transcode_input_buffer_pointers<unit_type> &scratch,
	basic_transcode_source<unit_type> source, unit_type const *next)
{
	// No engine result may escape the source range supplied for that call.
	if (next < source.first || next > source.last)
	{
	...
	::std::size_t minimum, ::std::size_t preferred)
{
					  ...
					  has_obuffer_basic_operations<output_ref>)
	{
		...
		unit_type *current{obuffer_curr(outref)};
		unit_type *end{obuffer_end(outref)};
		if (current != nullptr && end != nullptr &&
			static_cast<::std::size_t>(end - current) >= minimum)
		{
...
inline void commit_transcode_destination(
	...
	unit_type *next)
{
	...
	if (next < destination.first || next > destination.last)
	{
