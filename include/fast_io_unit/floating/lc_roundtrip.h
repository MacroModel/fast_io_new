...
constexpr char_type *lc_grouping_single_sep_ch_impl(::std::size_t const *grouping_base, ::std::size_t grouping_len,
													char_type *first, char_type *last, char_type *out_reverse,
													char_type replacement_ch) noexcept
{
	auto iter{last};
	::std::size_t i{};
	...
	else // decimal
	{
		...
		case 1:
		{
			...
			else
			{
				char_type buffer1[m10total_digits];
				auto buffer1_end{fixed_case0_full_integer<flt>(buffer1, m10, olength, real_exp)};
				return grouping_handle_buffer<m10total_digits>(buffer1, buffer1_end, iter, grouping_base,
															   grouping_len, thousands_sep_base, thousands_sep_len);
			}
		}
		case 2:
