...
// those explicit-output objects unchanged without a marker.  That experiment
 ...
 * this marker makes the source opaque before IO's compiler-constant query.
 * Clang 23 keeps both explicit-memory and default-stdout probes unchanged when
 * this callback alone is ordinary inline, so the marker is GCC-only.
 */
template <typename output_type>
struct print_lowered_components
{
	...
	inline constexpr void operator()(component_types &&...components) const
	{
		...
		{
			// Formatting ends at typed components. The public IO entry owns every
			// source-normalization and destination-emission policy from this point.
 ...
 * Removing only this marker from the otherwise complete chain loses the same
 * constant gate; it transports a value and introduces no output strategy.
 */
 ...
 * unprefixed facade.  Removing only this marker makes a caller-known scalar
 * opaque before the shared IO query.  The zero-argument overload has no value
 * to preserve and remains ordinary inline.
 */
template <basic_fixed_string format_literal, format_grammar grammar_type,
...
// Each value-carrying facade is the outermost measured GNU-frontend bridge link. The
// five code-unit domains and both grammars share the same implementation; no
// device or materialization policy is duplicated here.
